...
## 5. The transcript: a chain of snapshots (what PR #45 built)

This is the layer the T6 branch just finished. The mental model is closest to
**a tiny git for one entry's transcript**:
...
**Editing** (the store is built; the editor UI is T7): while you type, your text
sits in a `draft.json`. When the edit session ends (done, 90 s idle, 60 min cap,
or crash recovery), the draft is **spliced** against the text you were editing —
a diff figures out which pieces you kept and which you changed — and a new human
revision is minted. An unchanged draft mints nothing.
...
**revert** to it. All three are just new revisions; nothing is ever destroyed.

Details: [T6 design](plans/2026-08-03-t6-revision-chain-design.md) (§15/§15b =
as-built rulings), [T6 build plan](plans/2026-08-08-revision-chain-implementation-plan.md)

## Where the project is

```mermaid
flowchart LR
    M1["M1 capture ✅"] --> M2["M2 live transcript ✅"] --> M3["M3 journals + library ✅\n(search pending)"] --> T7["T7 editor UI\n◀ next"] --> T8["T8 retranscribe"] --> M4["M4 iCloud sync"] --> M5["M5 reading polish\n+ export + migration"]
```
...
markers with voice rendering. Just finished (PR #45, awaiting merge): the
revision-chain storage layer — everything in §5 except the editor UI itself.

Next, in order:

1. **T7 — the editor.** Edit the transcript (whole-revision accept v1), fix
   mis-tapped markers, audit log for metadata edits. Carries: typed-word
   correction for Swahili etc. (#37), keyboard-first editing on iPad/Mac,
   wiring the built-but-uncalled draft recovery (#41), storage visibility (#39).
2. **T8 — retranscription** from the m4a, plus contextual biasing so "LN" stops
   transcribing as "ellen" (#38).
