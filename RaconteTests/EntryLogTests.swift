...
final class EntryLogTests: XCTestCase {
    ...
    func testEntryMetadataFieldCountIsPinnedSoNewFieldsGetLogged() {
        XCTAssertEqual(Mirror(reflecting: EntryMetadata.defaults).children.count, 8,
                       ...
                       "never get a diff case of its own. `unknownFields` (#70) is a second: this " +
                       "change deliberately does not wire it into the audit-log differ — it is " +
                       "opaque JSON from a field this build cannot read, not an owner edit, so a " +
                       "diff of it would be noise rather than a real change to log. The CloudKit-" +
                       "side counterpart (an older build's `RemoteJournal`/`RemoteEntryFields` " +
                       "reading a newer build's CKRecord and losing any field it doesn't name) is " +
                       "the residual tracked on #70; this differ has no CloudKit-facing analogue " +
                       "of its own.")
    }
}
