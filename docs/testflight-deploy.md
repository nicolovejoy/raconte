...
Bump `CFBundleVersion` in **`project.yml`** (must be unique per upload; bump
`CFBundleShortVersionString` for real releases), run the suite, then:

⚠️ Not in `Raconte/Info.plist` — that file is GENERATED by XcodeGen, so a bump made
there is silently erased by the next `xcodegen generate`. That is how the build number
reset 2→1 mid-branch on 2026-08-24.
